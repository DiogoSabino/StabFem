...
for Re=Re_tab
    ...
    for om_ANALITIC=om_tab_ANALITIC
        ...
        Force_Coeff=-4*pi*1i*om_ANALITIC*(1+fa/beta);  %This is the coefficient
...
for Re=Re_tab
    ...
    scatter(log(dataRe.OMEGAtab(fv:lv,1)./(2*pi)),log ( (2*real(dataRe.Lift(fv:lv,1))) )  ) %Coefficients to tune in fuction of the points that one has...
    ...
    %%%%for index=startfrom:size(dataRe.OMEGAtab(fv:lv,1),1)
    %%%%    st=dataRe.OMEGAtab(index,1)/(2*pi);
    %%%%    str_latex=['(' num2str(st) ',' num2str((2*real(dataRe.Lift(index,1)))) ')'];
    %%%%    dlmwrite(filename_latex,str_latex,'delimiter', '','-append' )
    %%%%%end
end
